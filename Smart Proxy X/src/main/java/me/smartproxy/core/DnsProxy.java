...
public class DnsProxy implements Runnable {
    ...
    public void stop() {
        ...
        if (m_Client != null) {
            try {
                ...
                Log.e("DnsProxy", "Exception when closing m_Client", e);
            } finally {
                m_Client = null;
            }
        }
            ...
            if (question.Type == 1) {
                ...
                if (ProxyConfig.Instance.needProxy(question.Domain, realIP)) {
                    ...
                    if (ProxyConfig.IS_DEBUG)
                        Log.i("DnsProxy", String.format("FakeDns: %s=>%s(%s)\n", question.Domain,
                                CommonMethods.ipIntToString(realIP), CommonMethods.ipIntToString(fakeIP)));
                    return true;
                }
            }
        }
    ...
    private boolean interceptDns(IPHeader ipHeader, UDPHeader udpHeader, DnsPacket dnsPacket) {
        Question question = dnsPacket.Questions[0];
        if (ProxyConfig.IS_DEBUG)
            Log.d("DnsProxy", "DNS Query " + question.Domain);
        if (question.Type == 1) {
            if (ProxyConfig.Instance.needProxy(question.Domain, getIPFromCache(question.Domain))) {
                ...
                if (ProxyConfig.IS_DEBUG)
                    Log.d("DnsProxy", "interceptDns FakeDns: " +
                            question.Domain + " " +
